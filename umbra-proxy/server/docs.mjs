...
body{margin:0;padding:34px 28px 64px;color:#e2e9f4;
 background:radial-gradient(70% 46% at 10% -6%,rgba(157,140,255,.14) 0%,transparent 60%),
 radial-gradient(60% 40% at 96% -2%,rgba(111,227,255,.1) 0%,transparent 58%),#06070c;
 font:14px/1.65 ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;min-height:100vh}
main{max-width:920px;margin:0 auto}
.kicker{display:inline-block;font:10px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.18em;
 text-transform:uppercase;color:#c9c1ff;background:rgba(157,140,255,.1);border:1px solid rgba(157,140,255,.32);
 padding:6px 10px;border-radius:99px}
h1{font-size:30px;margin:14px 0 8px;letter-spacing:-.02em}
h2{font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#8b93a8;margin:32px 0 12px}
p.muted{color:#98a5bc;margin:0 0 18px;max-width:62ch}
a{color:#8fdcff}
form{display:flex;gap:8px;max-width:640px;margin-top:6px}
input{flex:1;background:rgba(4,6,10,.7);border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:12px 16px;
 color:#eaf2fb;font:13px ui-monospace,SFMono-Regular,Menlo,monospace;outline:0}
input:focus{border-color:rgba(157,140,255,.6);box-shadow:0 0 0 3px rgba(157,140,255,.1)}
button{border:1px solid rgba(157,140,255,.45);background:rgba(157,140,255,.16);color:#dcd8ff;border-radius:999px;
...
a.card:hover{background:rgba(255,255,255,.06);border-color:rgba(157,140,255,.45);transform:translateY(-1px)}
a.card b{display:block;color:#eef3fb;font-size:14px;margin-bottom:4px}
a.card span{color:#8b93a8;font-size:12px}
...
ol.res a{font-size:17px;color:#8fdcff;text-decoration:none}
ol.res a:hover{text-decoration:underline}
ol.res .from{font:11px ui-monospace,monospace;color:#9d8cff;margin:3px 0 5px;word-break:break-all}
ol.res p{margin:0;color:#9aa7bf;font-size:13px}
.facts{list-style:none;margin:0;padding:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:9px}
.facts li{border-left:2px solid rgba(157,140,255,.5);padding:3px 0 3px 12px;font-size:12.5px;color:#9aa7bf}
.facts b{color:#d9d4ff}
pre{background:rgba(0,0,0,.36);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:13px 14px;
 overflow:auto;font-size:12px;color:#bccbe0}
...
th{font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#8b93a8}
.warn{color:#ffcf8b}
...
.kv b{display:block;font:22px/1.1 ui-monospace,monospace;color:#a9ecff}
.kv span{font-size:10px;letter-spacing:.13em;text-transform:uppercase;color:#7f8ba0}
`;
...
export function portalDoc() {
  ...
  <ul class="facts">
    ...
    <li><b>redirects</b> chains are followed in the same tab like a browser; the bar shows the final address</li>
...
export function labIndexDoc() {
  const rows = [
    ['umbra://lab/redirect', 'synthetic 302', 'default policy follows it in-tab; strict policies hold it for review'],
    ['umbra://lab/samehost', 'same-host 301', 'must be followed silently: one tab, no notice'],
    ['umbra://lab/meta', 'meta refresh, 3s', 'same-tab navigation after the delay, like a browser'],
...
X-Umbra-Meta: base64url(json)                 final url, status, chain, kind</pre>
  ...
  <ul>
    ...
    <li>A document redirect chain is followed in-tab and the final address wins — never a 3xx to the client. Strict policies pause on a review capsule instead of moving the frame.</li>
